# Slimefun 5 (Unofficial extension/replacement/modified version of Slimefun 4)
*Looking for the download link? [**Click here**](https://modrinth.com/plugin/slimefun5)* 

Slimefun is a plugin which aims to turn your Spigot Server into a modpack without ever installing a single mod. It offers everything you could possibly imagine. From Backpacks to Jetpacks! Slimefun lets every player decide on their own how much they want to dive into Magic or Tech.<br>
We got everything from magical wands to nuclear reactors.<br>
We feature a magical altar, an electric power grid and even item transport systems.

This project originally started back **in 2013** and has grown ever since.<br>
From one single person working on this plugin back then, we grew to a community of thousands of players and hundreds of contributors to this project.<br>


### Quick navigation
* **[:floppy_disk: Download Slimefun5](#floppy_disk-download-slimefun-5)**
* **[:framed_picture: Screenshots](#framed_picture-screenshots)**
* **[:headphones: Discord Support Server](#headphones-discord)**
* **[:beetle: Bug Tracker](https://github.com/Slimefun/Slimefun4/issues)**
* **[:open_book: Wiki](https://github.com/Slimefun/Slimefun4/wiki)**
* **[:interrobang: FAQ](https://github.com/Slimefun/Slimefun4/wiki/FAQ)**

## :floppy_disk: Download Slimefun 5
Slimefun requires your Minecraft Server to be running on [Spigot](https://spigotmc.org/), [Paper](https://papermc.io/) or on any fork of these and have installed [MrLibCore](https://modrinth.com/plugin/mrlibcore).<br>
(See also: [How to install Slimefun](https://github.com/Slimefun/Slimefun4/wiki/Installing-Slimefun))

Slimefun 5 can be downloaded **for free** on modrinth.<br>


| | Slimefun 5 | Slimefun 4 |
| ------------------ | -------- | -------- |
| **Minecraft version(s)** | :video_game: **1.21.x** | :video_game: **1.16.\* - 1.20.\*** |
| **Java version** | :computer: **Java 21 (or higher)** | :computer: **Java 16 (or higher)** |
| **automatic updates** | :x: | :x: (Support End) |
| **frequent updates** | :heavy_check_mark: | :x: (Support End) |
| **latest content** | :heavy_check_mark: | :x: |
| **Discord support** | :heavy_check_mark: | :x: |
| **Bug Reports** | :heavy_check_mark: | :x: |
| **Slimefun 4 Addon Support** | ***NOT ALWAYS*** | :heavy_check_mark: |
| **testing before release** | :x: | :heavy_check_mark: |
| **Download links** | :floppy_disk: ***COMING SOON*** | :floppy_disk: **[download "stable"](https://blob.build/project/Slimefun4/RC)** |

#### :star: Highlighted Articles
* [What is Slimefun? *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [How to install Slimefun *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [Addons for Slimefun 5 *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [How to create an Addon for Slimefun 5 *** COMING SOON ***>](https://slimefun5.mrneznamy.eu)
* [Getting Started *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [Frequently Asked Questions *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [Common issues *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [Help us expand the Wiki! *** COMING SOON ***](https://slimefun5.mrneznamy.eu)
* [Help us translate Slimefun! *** COMING SOON ***](https://slimefun5.mrneznamy.eu)

## :exclamation: Disclaimers
We do not collect any personal information from you but there are some services that may gather or download some form of data.

Slimefun 5:
1. May or may not guarantee long-term support for older versions of Minecraft!
2. Does not guarantee 100% compatibility with the original addons for Slimefun 4.
3. Support may end overnight!
4. Is not my official plugin, it is just a replacement/extension/modified version of the original Slimefun 4 by TheBusyBiscuit and the community.
5. May contain many bugs, so use it at your own risk.

Additionally the plugin connects to [textures.minecraft.net](https://www.minecraft.net/en-us) to retrieve the Minecraft skins of our contributors (if possible).<br>

*Note that Slimefun is not associated with `Mojang Studios` or `Minecraft`.*
